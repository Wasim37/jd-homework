...
def main(train_file,
         ...
         max_grad_norm=10.0,
         checkpoint=None):
    bert_tokenizer = BertTokenizer.from_pretrained(os.path.join(
        root_path, 'lib/bert/vocab.txt'),
                                                   do_lower_case=True)
    ...
    patience_counter = 0
    for epoch in range(start_epoch, epochs_count + 1):
        ...
        print("-> Vliad time: {:.4f}s, loss = {:.4f}, accuracy: {:.4f}%".format(epoch_time, epoch_loss, (epoch_accuracy * 100)))
